...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['message'], $_POST['receiver_id'])) {
    ...
    if ($message) {
        ...
        createNotification($pdo, $receiverId, "New message from " . ($_SESSION['user_name'] ?? 'A patient'));
...
// For new message: list doctors
$doctors = $pdo->query("
    SELECT d.id AS doctor_id, u.id AS user_id, d.first_name, d.last_name, d.specialization
...
<div class="row">
    <div class="col-md-4">
                    ...
                    <div class="list-group list-group-flush">
                        ...
                        <a href="?user=<?= $c['other_user_id'] ?>" class="list-group-item list-group-item-action <?= ($otherUserId == $c['other_user_id']) ? 'active' : '' ?>">
                            <div class="d-flex w-100 justify-content-between">
                                <strong>Dr. <?= htmlspecialchars($c['first_name'] . ' ' . $c['last_name']) ?></strong>
        ...
        <div class="card shadow">
            <div class="card-header"><h5 class="mb-0"><i class="bi bi-envelope-plus"></i> New Message</h5></div>
            <div class="card-body">
                <form method="POST">
                    <div class="mb-2">
                        <select name="receiver_id" class="form-select" required>
                            ...
                            <option value="<?= $doc['user_id'] ?>">Dr. <?= htmlspecialchars($doc['first_name'] . ' ' . $doc['last_name']) ?> (<?= htmlspecialchars($doc['specialization']) ?>)</option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="mb-2">
    ...
    <div class="col-md-8">
        <div class="card shadow">
            <div class="card-header">
                <h5 class="mb-0">
                    <?php if ($otherUser): ?>
                        <i class="bi bi-person-circle"></i> Dr. <?= htmlspecialchars($otherUser['first_name'] . ' ' . $otherUser['last_name']) ?>
                    <?php else: ?>
            ...
            <div class="card-body" style="min-height: 400px; max-height: 500px; overflow-y: auto;">
                ...
                <?php if (!$otherUser): ?>
                    <p class="text-muted text-center mt-5">Select a conversation or send a new message.</p>
